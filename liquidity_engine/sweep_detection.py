...
class SweepDetector:
    def detect_sweeps(self, df: pd.DataFrame) -> List[Dict]:
        ...
        swings = df[(df['swing_high'].notna()) | (df['swing_low'].notna())]
        
        for i in range(1, len(df)):
            current_row = df.iloc[i]
            # Look for recent swings before current timestamp
            recent_swings = swings[swings['timestamp'] < current_row['timestamp']]
            
            if recent_swings.empty:
                continue
                
            last_high = recent_swings['swing_high'].dropna().iloc[-1] if not recent_swings['swing_high'].dropna().empty else None
            last_low = recent_swings['swing_low'].dropna().iloc[-1] if not recent_swings['swing_low'].dropna().empty else None
