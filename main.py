...
def main():
    ...
    zone_mapper = LiquidityZoneMapper(threshold=config.get("liquidity.sr_clustering_threshold", 0.002))
    ...
    # 3. Perform Analysis
    symbols = config.get("data_collection.forex_symbols", []) + \
              config.get("data_collection.crypto_symbols", [])
    ...
    for symbol in symbols:
        ...
        if not df.empty:
            ...
            logger.info(f"Running Liquidity Mapping for {symbol}...")
    ...
    journal.log_entry(
        content=f"System run completed. Analyzed {len(symbols)} symbols. Top correlation: {top_corrs[0]['pair'] if top_corrs else 'N/A'}",
        category="System",
        tags="auto-run"
    )
