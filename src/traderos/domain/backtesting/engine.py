    ...
    def run_backtest(self, strategy, df: pd.DataFrame) -> dict:
        ...
        for i in range(1, len(df)):
            ...
            if position != 0 and signal != position:
                # Close position
                exit_price = (
                    price * (1 - self.spread) if position == 1 else price * (1 + self.spread)
                )
            ...
            equity_curve.append(
                {"timestamp": current_row["timestamp"].isoformat(), "equity": current_equity}
            )
    ...
    def _save_results(self, strategy_name, symbol, start, end, metrics, equity_curve):
        ...
        cursor.execute(
            ...
            INSERT INTO backtest_results
                (strategy_id, symbol, start_date, end_date, metrics_json, equity_curve_json)
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            (
                strategy_id,
