...
def register_operator_endpoints(router: APIRouter, orch_provider: OrchestratorProvider) -> None:
    ...
    # --- synthetic probes (run every 30s in prod; open for monitoring) ---
    ...
    def _run_broker_probe(orch: TradingOrchestrator) -> ProbeResult:
        ...
        In LIVE mode the probe is deliberately read-only: a cyclic real-money
        order round-trip (even placed+cancelled) is never acceptable, so the
    ...
    def probe_broker() -> ProbeResult:
        """Synthetic probe: place + cancel a limit order through the broker.

        Returns ok=False when latency > 1000ms or any error.
        Run every 30s from monitoring; alerts on ok=False.
        """
        return _run_broker_probe(orch_provider())

    @router.get("/probes", dependencies=[Depends(require_read)], response_model=ProbesSummary)
    def probes_summary() -> ProbesSummary:
        """Aggregate all probe results for monitoring dashboards."""
        return ProbesSummary(broker=_run_broker_probe(orch_provider()))
