...
class BinanceCollector(BaseCollector):
    ...
    def fetch_data(self, symbol: str, days: int = 30) -> pd.DataFrame:
        ...
        logger.info("Fetching Binance data for %s (last %d days)", symbol, days)
        try:
            since = self.exchange.parse8601((datetime.now(UTC) - timedelta(days=days)).isoformat())
            all_ohlcv = []
            ...
            return df
        except (ValueError, ccxt.NetworkError) as e:
            logger.error("Error fetching Binance data for %s: %s", symbol, e)
            return pd.DataFrame()
...
class YFinanceCollector(BaseCollector):
    ...
    def fetch_data(self, symbol: str, days: int = 30) -> pd.DataFrame:
        logger.info("Fetching yfinance data for %s (last %d days)", symbol, days)
        try:
            # yfinance uses symbols like 'EURUSD=X' for Forex
            yf_symbol = f"{symbol}=X" if len(symbol) == 6 else symbol
            logger.info("Using yfinance symbol: %s", yf_symbol)
            ...
            logger.error("Error fetching yfinance data for %s: %s", symbol, e)
            return pd.DataFrame()
...
class MockDataCollector(BaseCollector):
    ...
    def fetch_data(self, symbol: str, days: int = 30) -> pd.DataFrame:
        logger.info("Generating mock data for %s", symbol)
        periods = days * 24
