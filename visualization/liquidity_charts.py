...
class LiquidityVisualizer:
    ...
    def plot_liquidity_map(self, df: pd.DataFrame, zones: list, sweeps: list, symbol: str):
        ...
        for zone in zones:
            ...
            plt.axhline(y=zone['price_level'], color=color, linestyle='--', alpha=0.3, 
                        label=f"{zone['zone_type']} (Str: {zone['strength']})")

        # Plot Liquidity Sweeps
        for sweep in sweeps:
            plt.scatter(sweep['timestamp'], df.loc[df['timestamp'] == sweep['timestamp'], 'close'], 
                        marker='x', color='purple', s=100, label='Liquidity Sweep')
