...
**Generated:** 2026-08-10 · **Branch:** `main` · **HEAD:** pending Sprint 32 push
...
| G-06 | Test realism / oracle | 85 | MEDIUM | 1572 tests, 92%+ coverage, conformance + dependency-direction tests, forced-disconnect + supervision + secret-hygiene + causal-replay + governance drills, plus **oracle conformance drill**: engine reproduces committed reference PnL on the frozen dataset AND the withheld window to tolerance 1e-4 (`sprint27_oracle_conformance.log`); **WP13 runs all 15 credential-free drills as a CI job** (`evidence-drills` in `.github/workflows/ci.yml`) — the build fails if any drill regresses; 8 credential/network/instance-gated drills are asserted out of the deterministic drill job (credential-gated ones stay operator-run; network-gated ones are exercised by the test suite when reachable) | Reference PnL oracle only covers the frozen G-06 candles, not every strategy; the CI drill job covers credential-free drills only (credential-gated ones stay operator-run) | A known strategy run through a frozen dataset reproduces a committed reference PnL to a stated tolerance — **proven 2/2**; evidence drills now enforced in CI — **closed (WP13)** |
...
- **WP13 — the evidence drills are now a CI gate.** All 15 credential-free
  drills run in the `evidence-drills` CI job and the build fails if any drill
  regresses; the 8 key-gated drills (live credentials / managed Vault /
